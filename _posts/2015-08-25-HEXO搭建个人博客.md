...
1.将角色基本模型替换至Ｍesh中的Skeletal Mesh处。并调整角色模型比例大小及初始方向。

2.添加SpringArm组件，并在组件下添加Camera,用于角色视角控制。

创建Play Controller，并命名为BP_Control_Hero。

创建Game Mode Base，并命名为BP_Mode_Hero，打开界面，将BP_Hero和BP_Control_Her添加其中。

在世界设置中，将Gamemode Override改为BP_Mode_Hero。
