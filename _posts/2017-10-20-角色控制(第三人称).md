...
2.创建Play Controller，并命名为BP_Control_Hero。

3.创建Game Mode Base，并命名为BP_Mode_Hero，打开界面，将BP_Hero和BP_Control_Her添加其中。

4.在世界设置中，将Gamemode Override改为BP_Mode_Hero。

### 移动功能实现
在BP_Hero使用input中添加的移动指令，来触发角色移动。详细蓝图如下：

![](/images/Pic/10.20/10.20_1.png)
